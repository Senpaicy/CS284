...
public class DoubleProbingTable<HashedObj> implements HashTable<HashedObj> {
    ...
    private int InternalHash2(HashedObj x){
        return 97 - Math.abs((x.hashCode())%97);
    }
    ...
    /*
     *  Boiler plate code below, read through it if you like. nty
     */
    ...
    public DoubleProbingTable(int size){
        this.size = size;
        array_ = new ArrayList<>(size);
        makeEmpty();
    }
    public DoubleProbingTable(){
        size = 101;
        array_ = new ArrayList<>(size);
        makeEmpty();
    }
